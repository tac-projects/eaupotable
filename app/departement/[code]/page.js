...
import { CTR_TEST_SLUGS, ctrTestDeptDescription } from '../../../lib/ctr-test';
...
export async function generateMetadata({ params }) {
  ...
  const baseDescription = `Votre eau du robinet est-elle saine ? Score moyen : ${scoreString}/10. Découvrez le classement et le bilan complet (Calcaire, Nitrates & PFAS) des ${cityCount} communes de ${deptName}.`;
  // Test CTR P2 : variante description sur le lot figé (title inchangé).
  const description = CTR_TEST_SLUGS.has(`/departement/${code}`)
    ? ctrTestDeptDescription({ deptName, scoreString, cityCount })
    : baseDescription;
