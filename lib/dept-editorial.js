...
// Lien vers une page réseau : si le distributeur correspond à une installation
// unique (UDI), on renvoie son slug ; sinon null (pas de lien fiable).
function findReseauForDistributor(udiToSlug, name, cities) {
  if (!name) return null;
  const target = name.trim().toLowerCase();
  const matches = new Set();
  for (const c of cities) {
    const ri = c.reseauInfo;
    if (!ri) continue;
    const inst = (ri.installation || ri.maitreOuvrage || '').trim().toLowerCase();
    const dist = (c.meta?.nom_distributeur || '').trim().toLowerCase();
    if (inst === target || dist === target) {
      if (c.reseau && udiToSlug[c.reseau]) matches.add(udiToSlug[c.reseau]);
    }
  }
...
// ---------------------------------------------------------------------------
export function computeDeptEditorial(deptData, udiToSlug = {}) {
  ...
  const distributors = [...distMap.entries()]
    ...
    .map(([name, count]) => ({
      ...
      slug: findReseauForDistributor(udiToSlug, name, cities),
    }));

  // 3. Prix (€/m³) — avec la commune concernée pour le minimum et le maximum
  const pricedCities = cities
    .map(c => ({ name: c.cityName, slug: c.slug, price: c.prix?.total ?? c.prix?.aep }))
    .filter(c => typeof c.price === 'number' && !isNaN(c.price));
  const priceStats = pricedCities.length
    ? (() => {
        const minCity = pricedCities.reduce((a, b) => (b.price < a.price ? b : a));
        const maxCity = pricedCities.reduce((a, b) => (b.price > a.price ? b : a));
        return {
          min: minCity.price,
          max: maxCity.price,
          avg: pricedCities.reduce((sum, c) => sum + c.price, 0) / pricedCities.length,
          count: pricedCities.length,
          minCity: { name: minCity.name, slug: minCity.slug },
          maxCity: { name: maxCity.name, slug: maxCity.slug },
        };
      })()
    : null;
