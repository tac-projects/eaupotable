import nationalData from '@/app/data/france.json';
import crystalEngine from './crystal-engine';
import siseParamCodes from './sise-param-codes';

const { SISE_CODES } = siseParamCodes;

export const calculateCrystalScore = crystalEngine.calculateCrystalScore;
...
export const PARAM_ICONS = {
    ...
    conformity: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>'
};

export const RANGES = {
    nitrates: [5, 20, 50],
    pesticides: [0.1, 0.25, 0.5],
...
export const NATIONAL_STATS = nationalData.averages;
...
/**
 * UNIQUE CALCULATION ENGINE — cf. lib/crystal-engine.js (source de vérité).
 * Ce fichier ré-exporte simplement le moteur canonique (CJS partagé avec les
 * scripts de build node). Ne jamais ré-implémenter le calcul ici.
 */
...
export async function harvestWaterData(cityName) {
    try {
        ...
        const getParam = (codes, keywords, requiredUnits = []) => {
            ...
            const isBact = SISE_CODES.microbiology.some(c => codes.includes(c));
        ...
        const stats = {
            nitrates: getParam(SISE_CODES.nitrates, []),
            ph: getParam(SISE_CODES.ph, ["ph", "potentiel hydrogene"], ["ph"]),
            hardness: getParam(SISE_CODES.hardness, ["hydrotimetrique", "durete", "th"]),
            chlorine: getParam(SISE_CODES.chlorine, ["chlore libre", "chlore total"]),
            conductivity: getParam(SISE_CODES.conductivity, ["conductivite"], ["µS", "siemens", "us/cm"]),
            turbidity: getParam(SISE_CODES.turbidity, ["turbidite"], ["NFU", "NTU", "SiO2"]),
            pesticides: getParam(SISE_CODES.pesticides, ["pesticide"]),
            pfas: getParam(SISE_CODES.pfas, ["pfas", "perfluoro"]),
            microbiology: getParam(SISE_CODES.microbiology, ["escherichia", "enterocoques", "coliformes"]),
            iron: getParam(SISE_CODES.iron, ["fer total", "fer dissous"]),
            manganese: getParam(SISE_CODES.manganese, ["manganese"]),
            ammonium: getParam(SISE_CODES.ammonium, ["ammonium"]),
            copper: getParam(SISE_CODES.copper, ["cuivre"]),
            cot: getParam(SISE_CODES.organic_carbon, ["organique total", "cot"])
        };

        const conclusion = reports[0].conclusion_conformite_prelevement || "";
        const arsConform = conclusion.toLowerCase().includes("conforme") && !conclusion.toLowerCase().includes("non conforme");
        const isConform = arsConform && !crystalEngine.hasLimitExceedance(stats);
        const nonConformReport = reports.find(r => (r.conclusion_conformite_prelevement || "").toLowerCase().includes("non conforme aux limites"));
        const meta = nonConformReport ? {
            ...reports[0],
            conclusionNonConforme: nonConformReport.conclusion_conformite_prelevement,
            dateNonConforme: new Date(nonConformReport.date_prelevement).toLocaleDateString('fr-FR')
        } : reports[0];
        ...
        if (allReseauCodes.length > 0) {
            ...
            const fallbackConfig = {
                nitrates: SISE_CODES.nitrates.join(','),
                ph: SISE_CODES.ph.join(','),
                hardness: SISE_CODES.hardness.join(','),
                chlorine: SISE_CODES.chlorine.join(','),
                conductivity: SISE_CODES.conductivity.join(','),
                turbidity: SISE_CODES.turbidity.join(','),
                pesticides: SISE_CODES.pesticides.join(','),
                pfas: SISE_CODES.pfas.join(','),
                microbiology: SISE_CODES.microbiology.join(','),
                iron: SISE_CODES.iron.join(','),
                manganese: SISE_CODES.manganese.join(','),
                ammonium: SISE_CODES.ammonium.join(','),
                copper: SISE_CODES.copper.join(','),
                cot: SISE_CODES.organic_carbon.join(',')
            };
                    ...
                    if (bRes.ok) {
                        ...
                        if (bData.data && bData.data.length > 0) {
                            missingKeys.forEach(key => {
                                ...
                                const preferred = key === 'pesticides' ? bData.data.find(m => String(m.code_parametre) === '6276') : null;
                                const match = preferred || bData.data.find(m => codes.includes(String(m.code_parametre)));
                                if (match) {
        ...
        return { stats, isConform, crystal, meta, cityName: officialCityName };
    } catch (e) {
...
export function getParameterStatus(key, val) {
    ...
    switch(key) {
        ...
        case "pesticides":
            ...
            if (p <= 0.1) return { class: "status-excellent", statusLabel: "Excellent", subtitle: "Traces infimes", status: "perfect" };
            if (p <= 0.25) return { class: "status-good", statusLabel: "Bon", subtitle: "Présence de résidus", status: "perfect" };
            if (p <= 0.5) return { class: "status-warning", statusLabel: "Médiocre", subtitle: "Limite de conformité", status: "warning" };
...
/**
 * Position (%) du marqueur sur la jauge. Interpolation linéaire par zone :
 * les bornes des zones correspondent exactement aux labels affichés
 * (0 / 25 / 50 / 75 / 100 %). Une valeur nulle va à gauche, une valeur au-delà
 * du max de l'échelle est poussée complètement à droite (100 %).
 */
export function getMarkerPosition(key, val, range, status) {
    if (key === "bacteria" || key === "microbiology") {
        return status?.status === "perfect" ? 10 : 90;
    }
    const n = parseValue(val);
    if (!range || isNaN(n)) return 50;

    if (CENTERED_PARAMS.includes(key)) {
        const [c1, w1, g1, g2, w2, c2] = range;
        const lo = c1 - (w1 - c1);
        const hi = c2 + (c2 - w2);
        if (n <= lo) return 0;
        if (n <= c1) return 11 * (n - lo) / (c1 - lo);
        if (n <= w1) return 11 + 11 * (n - c1) / (w1 - c1);
        if (n <= g1) return 22 + 11 * (n - w1) / (g1 - w1);
        if (n <= g2) return 33 + 34 * (n - g1) / (g2 - g1);
        if (n <= w2) return 67 + 11 * (n - g2) / (w2 - g2);
        if (n <= c2) return 78 + 11 * (n - w2) / (c2 - w2);
        if (n >= hi) return 100;
        return 89 + 11 * (n - c2) / (hi - c2);
    }

    const [b1, b2, b3] = range;
    const scaleMax = b3 + (b3 - b2);
    if (n <= 0) return 0;
    if (n <= b1) return 25 * n / b1;
    if (n <= b2) return 25 + 25 * (n - b1) / (b2 - b1);
    if (n <= b3) return 50 + 25 * (n - b2) / (b3 - b2);
    if (!(scaleMax > b3) || n >= scaleMax) return 100;
    return 75 + 25 * (n - b3) / (scaleMax - b3);
}
