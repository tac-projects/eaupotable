...
export function parseValue(val) {
    ...
    // Supprime tout ce qui n'est pas chiffre, point ou virgule (ex: <, >, mg/L, etc.)
...
export function calculateCrystalScore(s, isConform) {
    ...
    // 1. NITRATES (Exigence renforcée)
    if (s.nitrates) {
        const n = parseValue(s.nitrates.val);
        if (n > 50) score -= 8;
        else if (n > 40) score -= 3.0 + (n - 40) * 0.4;
        else if (n > 20) score -= 0.6 + (n - 20) * 0.15; // Plus exigeant dès 20mg/L
        else if (n > 10) score -= 0.2 + (n - 10) * 0.04;
    }

    // 2. PESTICIDES (Proportionnel)
    if (s.pesticides) {
        const p = parseValue(s.pesticides.val);
        if (p > 0.1) score -= 6;
        else if (p > 0.05) score -= 2.5;
        else if (p > 0) {
            score -= (p * 25); // Un peu plus sévère sur les traces
        }
    }
    ...
    // 4. CHLORE (Exigence Goût)
    if (s.chlorine) {
        const c = parseValue(s.chlorine.val);
        if (c > 0.5) score -= 1.2; // Sanction plus forte sur le goût Javel
        else if (c > 0.1) score -= 0.6;
    }

    const others = [s.iron, s.manganese, s.copper, s.ammonium, s.turbidity, s.cot];
    others.forEach(stat => {
        if (!stat) return;
    ...
    if (score >= 9.7) {
        ...
        explanation = "Une pureté totale, digne des meilleures eaux de source.";
    } else if (score >= 9.2) {
        label = "EXCELLENTE";
        explanation = "Qualité remarquable, quasiment aucun polluant détecté.";
    } else if (score >= 8.5) {
        label = "TRÈS BONNE";
        explanation = "Très bonne qualité globale, malgré quelques paramètres mineurs.";
    } else if (score >= 7.0) {
        label = "SATISFAISANTE";
        explanation = "Une eau saine qui respecte les équilibres fondamentaux.";
    } else if (score < 5.0) {
        label = "DÉGRADÉE";
        explanation = "La qualité de l'eau est impactée par des paramètres critiques.";
    } else if (score < 7.0) {
        label = "MÉDIOCRE";
        explanation = "Qualité passable présentant plusieurs points de vigilance.";
    }
  ...
  try {
    ...
    const getParamLegacy = (codes, keywords, requiredUnits = []) => {
      const match = reports.find(r => {
        ...
        // Filtrage spécifique : On veut tout sauf la température qui parasite les recherches de pH/TH
        const isTemp = label.includes("temp") || label.includes("t°") || unit.includes("°c") || unit.includes("celsius");
        if (isTemp) return false;
        // On autorise °f (calcaire) même s'il y a un °
        ...
        const isWordMatch = keywords.some(kw => label.includes(kw.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")));
      ...
      return { val: rawVal !== null ? `${rawVal}` : '--' };
    };

    const stats = {
      nitrates: getParamLegacy([1340, 1342], []),
      ph: getParamLegacy([1301, 1302], ["ph", "potentiel hydrogene"], ["ph"]),
      hardness: getParamLegacy([1345, 2708], ["hydrotimetrique", "durete", "th"]),
      chlorine: getParamLegacy([1399, 1398], ["chlore libre", "chlore total"]),
      conductivity: getParamLegacy([1302, 1303], ["conductivite"], ["µS", "siemens", "us/cm"]),
      turbidity: getParamLegacy([1305], ["turbidite", "turb"]),
      iron: getParamLegacy([1393, 1374], ["fer total", "fer dissous"]),
    ...
    // Fallback Réseau Généralisé : Repêchage ciblé pour toute donnée manquante (Sniper SEO)
    ...
    if (reseau) {
      ...
      const missingKeys = Object.keys(fallbackConfig).filter(key => !stats[key] || stats[key].val === '--');
      
      if (missingKeys.length > 0) {
        await Promise.all(missingKeys.map(async (key) => {
          try {
            const rRes = await fetch(`https://hubeau.eaufrance.fr/api/v1/qualite_eau_potable/resultats_dis?code_reseau=${reseau}&code_parametre=${fallbackConfig[key]}&size=1&sort=desc`);
            const rData = await rRes.json();
            if (rData.data && rData.data.length > 0 && rData.data[0].resultat_numerique !== null) {
              stats[key] = { val: `${rData.data[0].resultat_numerique}` };
            }
          } catch (e) {}
        }));
      }
    ...
    return {
      score: crystal.final,
      label: crystal.label,
      statusClass: crystal.statusClass,
      cityName
    };
  } catch (e) {
...
export function getParameterStatus(key, val) {
    ...
    if (key === "bacteria") {
        ...
        if (lowerVal.includes("absence")) return { class: "status-excellent", statusLabel: "Sain", subtitle: "Aucun germe détecté", status: "perfect" };
    ...
    switch(key) {
        ...
        case "pfas":
            if (isNaN(n) || n === 0) return { class: "status-excellent", statusLabel: "Indétectable", subtitle: "Seuil de sécurité total", status: "perfect" };
            if (n <= 0.1) return { class: "status-good", statusLabel: "Conforme", subtitle: "Sous la limite européenne", status: "perfect" };
            return { class: "status-critical", statusLabel: "Vigilance", subtitle: "Dépassement de seuil", status: "critical" };
        default:
