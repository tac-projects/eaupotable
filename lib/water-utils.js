...
export function calculateCrystalScore(s, isConform) {
    ...
    if (s.nitrates) {
        ...
        if (n > 50) score -= 8;
        else if (n > 40) score -= 4 + (n - 40) * 0.4; // Progression entre 40 et 50
        else if (n > 25) score -= 1 + (n - 25) * 0.2; // Progression entre 25 et 40
        else if (n > 10) score -= 0.3 + (n - 10) * 0.05; // Progression entre 10 et 25
    }
    ...
    const others = [s.iron, s.manganese, s.copper, s.ammonium, s.turbidity];
    others.forEach(stat => {
        if (!stat) return;
    ...
    if (score >= 8.5) statusClass = "status-excellent";
    else if (score < 5) statusClass = "status-critical";
    else if (score < 8) statusClass = "status-warning";
...
export async function fetchCitySummary(cityName) {
  try {
    ...
    const stats = {
      ...
      pesticides: getParamLegacy([1107, 1667, 6272, 6273, 6274, 6275, 6276, 6277, 6278, 6279, 6280, 7149, 7150], ["pesticide"]),
