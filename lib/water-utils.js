...
/**
 * Position (%) du marqueur sur la jauge. Jauge qualitative à 4 zones.
 * Au-delà du seuil réglementaire (range[2]), interpolation jusqu'à la borne
 * haute de l'échelle (range[2] + (range[2] - range[1])) puis plafonnement à
 * 100 % : une valeur hors échelle est poussée complètement à droite.
 * Une valeur nulle est un point valide (15 %), pas une absence (50 %).
 */
export function getMarkerPosition(key, val, range, status) {
    ...
    if (!(scaleMax > b3)) return 100;
    return Math.min(100, 75 + ((n - b3) / (scaleMax - b3)) * 25);
}
