...
export function calculateCrystalScore(s, isConform) {
    ...
    // Fonction utilitaire de détection intelligente
    const isDetected = (p, threshold = 0) => {
        ...
        // On ne pénalise que si la valeur dépasse le seuil de vigilance
    ...
    // 1. MICROBIOLOGIE
    if (s.bacteria && isDetected(s.bacteria)) {
        score -= 10;
    }

    // 2. NITRATES (Vigilance > 10)
...
export async function fetchCitySummary(cityName) {
  try {
    const isMetropole = ['lyon', 'paris', 'marseille', 'bordeaux', 'toulouse', 'nantes', 'lille', 'montpellier'].includes(cityName.toLowerCase());
    const size = isMetropole ? 5000 : 1000;
    ...
    // Fallback Réseau Généralisé : Repêchage ciblé pour toute donnée manquante (Sniper SEO Multi-Réseaux)
    ...
    if (allReseauCodes.length > 0) {
        ...
        // Exécution en parallèle pour la performance réseau
        await Promise.all(Object.keys(fallbackConfig).map(async (key) => {
            if (!stats[key] || stats[key].val === '--') {
                try {
                    const rRes = await fetch(`https://hubeau.eaufrance.fr/api/v1/qualite_eau_potable/resultats_dis?code_reseau=${reseauQuery}&code_parametre=${fallbackConfig[key]}&size=1&sort=desc`);
                    const rData = await rRes.json();
                    if (rData.data && rData.data.length > 0) {
                        const match = rData.data[0];
                        const rawVal = (match.resultat_alphanumerique && match.resultat_alphanumerique !== "null") ? match.resultat_alphanumerique : match.resultat_numerique;
                        if (rawVal !== null && rawVal !== undefined) {
                            stats[key] = { val: `${rawVal}` };
                        }
                    }
                } catch (e) {}
            }
