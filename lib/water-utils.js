...
export function calculateCrystalScore(s, isConform) {
    ...
    const others = [
        ...
        { val: s.turbidity, limit: 0.5, penalty: 1.5 }, // Sanction plus forte sur la turbidité visuelle
        { val: s.ph, limit: 9.0, penalty: 0.5, lowLimit: 6.5 },
        { val: s.ammonium, limit: 0.1, penalty: 0.5 },
        { val: s.copper, limit: 1.0, penalty: 0.5 }
    ];

    others.forEach(o => {
        if (!o.val) return;
        const n = parseValue(o.val.val);
        if (isNaN(n) || (n === 0 && o.val.val !== '--')) return;
...
export async function harvestWaterData(cityName) {
    try {
        ...
        // Vérification de sécurité avant parsing JSON
        if (!response.ok) {
            console.warn(`Hub'Eau Error: ${response.status} for ${cityName}`);
            return null;
        }
        const contentType = response.headers.get("content-type");
        if (!contentType || !contentType.includes("application/json")) {
            console.warn(`Hub'Eau returned non-JSON response for ${cityName}`);
            return null;
        }
        ...
        if (allReseauCodes.length > 0) {
            ...
            if (missingKeys.length > 0) {
                // Pour les métropoles, 10 000 est nécessaire car elles testent la microbio 100x plus que le reste
                const batchUrl = `https://hubeau.eaufrance.fr/api/v1/qualite_eau_potable/resultats_dis?code_reseau=${reseauQuery}&size=10000&sort=desc`;
        ...
        const officialCityName = rawName.toLowerCase().split('-').map(part => part.split(' ').map(sub => sub.charAt(0).toUpperCase() + sub.slice(1)).join(' ')).join('-');
