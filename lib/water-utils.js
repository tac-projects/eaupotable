...
export function calculateCrystalScore(s, isConform) {
    ...
    if (score >= 9.7) { label = "EXCEPTIONNEL"; explanation = "Une pureté totale, digne des meilleures eaux de source."; }
    else if (score >= 9.2) { label = "EXCELLENT"; explanation = "Qualité remarquable, quasiment aucun polluant détecté."; }
    else if (score >= 8.5) { label = "TRÈS BON"; explanation = "Très bonne qualité globale, malgré quelques paramètres mineurs."; }
    else if (score >= 7.0) { label = "SATISFAISANT"; explanation = "Une eau saine qui respecte les équilibres fondamentaux."; }
    else if (score < 5.0) { label = "DÉGRADÉ"; explanation = "La qualité de l'eau est impactée par des paramètres critiques."; }
