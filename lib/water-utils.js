...
export async function harvestWaterData(cityName) {
    // ARCHIVE BYPASS : On check d'abord notre base de donnée locale ultra-complète (2022-2026)
    try {
        const slug = cityName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, '-');
        // Pour l'instant on teste sur le 44
        const localRes = await fetch('/data/departments/44.json');
        if (localRes.ok) {
            const localData = await localRes.json();
            if (localData.cities[slug]) {
                const cityData = localData.cities[slug];
                // On injecte les moyennes départementales pour le duel
                return {
                    ...cityData,
                    initialDeptAvg: localData.deptInfo
                };
            }
        }
    } catch (e) {
        console.warn("Archiviste local non disponible, passage en mode API Hub'Eau...");
    }
