 ...
 *   - cause sanitaire avérée (bactérie, pesticides totaux > 0,5, PFAS > 0,1, nitrates ≥ 50)
...
const QUALITY_LIMITS = { nitrates: 50, pesticides: 0.5, pfas: 0.1 };

function exceedsLimit(p, limit) {
    return isDetected(p) && parseValue(p.val) > limit;
}

/**
 * Dépassement d'une limite de qualité sur la dernière valeur connue de chaque
 * paramètre. Permet de dériver la conformité sanitaire indépendamment du
 * verdict du dernier prélèvement global, qui peut ne pas couvrir un paramètre
 * (ex. pesticides mesurés en mars, contrôle global conforme en juin).
 */
function hasLimitExceedance(s) {
    const stats = s || {};
    return isDetected(stats.microbiology)
        || exceedsLimit(stats.nitrates, QUALITY_LIMITS.nitrates)
        || exceedsLimit(stats.pesticides, QUALITY_LIMITS.pesticides)
        || exceedsLimit(stats.pfas, QUALITY_LIMITS.pfas);
}
...
function calculateCrystalScore(s, isConform) {
    ...
    // 2. Pesticides — total des pesticides analysés (limite 0,5 µg/L)
    if (isDetected(stats.pesticides)) {
        score -= (parseValue(stats.pesticides.val) > 0.5 ? 4.0 : 1.5);
    }
    ...
    if (isConform === false) {
        ...
        const pestCrit = isDetected(stats.pesticides) && parseValue(stats.pesticides.val) > 0.5;
...
module.exports = { calculateCrystalScore, parseValue, hasLimitExceedance };
